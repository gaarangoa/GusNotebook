...
def main():
    ...
    # Generated by pandas.DataFrame(...)._repr_html_(), including its scoped CSS.
    table = (Path(__file__).parent / 'fixtures' / 'dataframe.html').read_text()
    nbformat.write(nbformat.v4.new_notebook(cells=[
        ...
        nbformat.v4.new_code_cell('answer = 42\nprint(answer)', execution_count=1, outputs=[
            nbformat.v4.new_output('display_data', data={'text/html': '<h1>Results</h1>' + table, 'text/plain': 'Treatment: 67%'}),
            nbformat.v4.new_output('display_data', data={'text/markdown': '# Output heading\n## Summary\n### Detail'}),
            nbformat.v4.new_output('stream', name='stdout', text='Printed output\n'),
            nbformat.v4.new_output('execute_result', execution_count=1, data={'text/plain': '42'})]),
    ...
    with sync_playwright() as playwright:
        ...
        def expect_font_size(size):
            for selector in ['body', '.app-header .tab', '.file-row', '.strip-head', '.toolbar .tb',
                             '#agent-kind', '.term-btn', '.md-rendered',
                             '.cm-content', '.cm-lineNumbers .cm-gutterElement', '.output.stream',
                             '.output.result', '.output.error', '.output-raw']:
                expect(page.locator(selector).first).to_have_css('font-size', f'{size}px')
            for level, scale in enumerate([2, 1.5, 1.25, 1, .875, .85], start=1):
                expect(page.locator(f'.md-rendered h{level}')).to_have_css('font-size', f'{size * scale:g}px')
                if level <= 3:
                    expect(page.locator(f'.output-html h{level}')).to_have_css('font-size', f'{size * scale:g}px')
            expect(page.locator('.md-rendered h3 code')).to_have_css('font-size', f'{size * 1.25:g}px')
            frame = page.frame_locator('iframe[data-output-frame]')
        ...
        expect(page.locator('#sidebar-terminal')).to_have_count(0)
        ...
        # Each rail button opens its own full-height section.
        assert page.locator('#skills svg').count() == 0
        for section in ['skills', 'sessions']:
            page.click('#sidebar-' + section)
            assert page.locator('#' + section + ' .strip-head').bounding_box()['height'] == 44
            expect(page.locator('#sidebar-' + section)).to_have_attribute('aria-pressed', 'true')
            expect(page.locator('#sidebar-files-pane')).not_to_be_visible()
        ...
        expect(page.locator('#skill-back')).to_be_visible()
        page.keyboard.press('Escape')
        page.locator('#sidebar-sessions').press('Enter')
        ...
        page.screenshot(path=str(screenshots / 'sidebar-sessions.png'))
        page.reload(wait_until='domcontentloaded')
        page.wait_for_function('booted && cells.length === 3 && cmViews.size === 2')
        expect(page.locator('#sessions')).to_be_visible()
        expect(page.locator('#sidebar-sessions')).to_have_attribute('aria-pressed', 'true')
        expect(page.locator('#sidebar-files-pane')).not_to_be_visible()
        page.click('#sidebar-sessions')
        expect(page.locator('#files')).not_to_be_visible()
        expect(page.locator('#sidebar-rail')).to_be_visible()
        expect(page.locator('#sidebar-sessions')).to_have_attribute('aria-pressed', 'false')
        page.locator('#sidebar-sessions').press('ArrowDown')
        expect(page.locator('#sidebar-skills')).to_be_focused()
        page.keyboard.press('Enter')
        expect(page.locator('#skills')).to_be_visible()
        expect(page.locator('#sessions')).not_to_be_visible()
        page.screenshot(path=str(screenshots / 'sidebar-skills.png'))
        page.evaluate('path => showInFileTree(path)', str(notebook))
        expect(page.locator('#sidebar-files-pane')).to_be_visible()
        expect(page.locator('#sidebar-files')).to_have_attribute('aria-pressed', 'true')
        expect(page.locator('.file-row.located')).to_be_visible()
        # Collapse and reopen Files without resetting the current directory.
        page.click('#sidebar-files')
        page.click('#sidebar-files')
        assert page.evaluate('fileState.path') == str(work)
        print('PASS: sidebar selection persists; rail keyboard navigation, collapse and file reveal work', flush=True)
        ...
        expect(page.locator('#skills')).to_be_visible()
        expect(page.locator('#agent-pane')).not_to_be_visible()
        page.locator('#toggle-terminal').press('Enter')
        expect(page.locator('#agent-pane')).to_be_visible()
        ...
        page.click('#sidebar-files')
        expect(page.locator('#agent-pane')).to_be_visible()
        print('PASS: Header terminal toggle preserves live terminals and operates independently of sidebar sections', flush=True)
        ...
        for width in [1024, 768, 390, 320]:
            ...
            page.keyboard.press('Escape')
            expect(page.locator('#sidebar-files')).to_be_focused()
            page.click('#sidebar-skills')
            expect(page.locator('#skills')).to_be_visible()
            assert page.locator('.left').bounding_box()['width'] > 200
            assert page.locator('#files').bounding_box()['x'] == 44
            page.click('#sidebar-sessions')
            expect(page.locator('#sessions')).to_be_visible()
            expect(page.locator('#skills')).not_to_be_visible()
            page.screenshot(path=str(screenshots / f'sidebar-{width}.png'))
            page.click('#sidebar-sessions')
            expect(page.locator('#files')).not_to_be_visible()
            if width < 820:
                page.click('#toggle-terminal')
        ...
        expect(page.locator('#out-' + page.evaluate('cells[1].id') + ' .output.stream')).to_have_text('42', timeout=60000)
